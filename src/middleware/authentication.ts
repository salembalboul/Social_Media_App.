...
if (!prefix || !token) {throw new appError("token not exist",400 );}

const signature = await getSignature(tokenType,prefix)   

if(!signature){ throw new appError("signature is not valid",400)}

const decoded =await decodedTokenAndFetchUser(token,signature)

if(!decoded){ throw new appError("token is not valid",400)}
